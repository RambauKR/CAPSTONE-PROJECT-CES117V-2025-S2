        ...
        try:
            ...
            if results[0].boxes is not None and results[0].boxes.id is not None:
                ...
                for box, track_id, class_id in zip(boxes, track_ids, class_ids):
                    ...
                    # Determine movement direction (IN/OUT)
                    if len(Group03Processor.track_history[track_id]) >= 2:
                        prev_y = Group03Processor.track_history[track_id][-2]
                        curr_y = center_y
                        mid_line = frame.shape[0] // 2
                        # Vehicle moving down (IN)
                        if prev_y < 300 <= curr_y and track_id not in Group03Processor.counted_ids_in:
                        ...
                        # Vehicle moving up (OUT)
                        elif prev_y > 650 >= curr_y and track_id not in Group03Processor.counted_ids_out:
