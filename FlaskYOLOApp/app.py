...
model = YOLO("C:/Users/Student1/Desktop/custom/custom/train/train/weights/best.pt")  # load the YOLOv8 nano model pre-trained on COCO
...
def generate_frames():
    ...
    while True:
        ...
        if not success:
            break  # if we can't get a frame, exit the loop

        # Apply thresholding to the frame
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)                # convert to grayscale
        _, thresh = cv2.threshold(gray, 128, 255, cv2.THRESH_BINARY)  # simple binary threshold

        processed_frame = thresh  # for now, we'll display the thresholded image

        # Store the last processed frame for snapshot (convert single-channel to BGR for consistency)
        last_frame = cv2.cvtColor(processed_frame, cv2.COLOR_GRAY2BGR)
        ...
        if not ret:
            continue  # skip this frame if there was an encoding error
