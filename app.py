...
# Try to import ultralytics YOLO (optional). If unavailable the app will still run.
model = None
try:
    from ultralytics import YOLO
    try:
        # load small model by default; this will download weights the first time
        model = YOLO('best.pt')
    except Exception:
        model = None
except Exception:
...
def generate_frames():
    ...
    while True:
        ...
        results = model.predict(frame, conf=0.25, verbose=False)  # run YOLO inference with 50% confidence threshold
...
def snapshot():
    ...
    cv2.imwrite(url_for('static', filename='snapshot.jpg')[1:], last_frame)
